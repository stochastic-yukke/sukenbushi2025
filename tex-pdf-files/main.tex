...
\lstset{
    language=Python,%
    basicstyle={\small\ttfamily},% 等幅フォント
...
\begin{dfn} \label{freshman}
    $x,y$がそれぞれどのような集合(またはクラス)$X,Y$の元であるか，$=$が$X,Y$上のどのような同値関係であるか，$+\colon(x,y)\mapsto x+y$がどのような演算であるか，$\cdot~^p$がどのような演算であるか，に関わらず，$(x + y)^p = x^p + y^p$という式を\textbf{Freshman's dream}と呼ぶことにする．また，これ以降，Freshman's dreamのことを\textbf{\textsf{FD}}と書く．
\end{dfn}
