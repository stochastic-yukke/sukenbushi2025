...
\begin{thm}[フェルマーの小定理]\label{fermat_little}
    $p$が素数で，$x \in \mathds{Z}$が$p$で割り切れなければ，$\boldsymbol{x^{p-1} \equiv 1 \pmod{p}}$．
\end{thm}

\begin{lemma}[\textsf{FD}]\label{mod_fresh}
    $p$が素数であるとき，$\boldsymbol{(x + y)^p \equiv x^p + y^p \pmod{p}}$である．
\end{lemma}
これは，二項定理と，任意の$0 < k < pを満たすk \in \mathds{Z}$に対して$\binom{p}{k}$が$p$の倍数であることを用いれば示すことができる。あとで出てくる補題\ref{ch_fresh}の証明に書いた方法とまったく同様である。それでは，この補題を用いて定理\ref{fermat_little}を示そう：
\begin{prf}
...
\begin{dfn}[体の標数]\label{ch_def}
    体$K$に対して，$K$の単位元を$1_K$とかく．$n \in \mathds{Z^{+}}$に対して，$n$を，$1_K$の$n$個の和として定義する．$n=0_K$($0_K$は加法の単位元)を満たす$n \in \mathds{Z^{+}}$が存在するとき，そのような$n$の最小値を$K$の\textbf{標数}と呼び，$\mathbf{ch}\boldsymbol{K}$とかく．
\end{dfn}

ここで，定義より，\textbf{標数は0か素数である}ことが示せる(ヒント：標数＞０を$lm\left(l,m\geqq 2, \, l,m \in \mathds{Z}\right)$とおくと，$K$の整域としての性質により$lm$の最小性に矛盾する)。

\begin{lemma}[\textsf{FD}]\label{ch_fresh}
    $K$が標数$p\, (>0)$の体で，$n \in \mathds{Z^{+}}$，$q=p^n$ならば，任意の$x,y \in K$に対して$\boldsymbol{(x+y)^{q}=x^q+y^q}$である．
\end{lemma}
...
\begin{thm}
    $K$が$\mathrm{ch}K=p$の体で，$n \in \mathds{Z}^{+}$に対し$q=p^n$ならば，$\mathrm{Frob_q}:K \ni x \mapsto x^q \in K$で定義された写像は体準同型である．
\end{thm}
...
\begin{dfn}\footnote{数字の上にバーをつけているのは，$n$で割ったあまりについての同値類であることを明確にするため。また，この定義は環論の「イデアル」を用いた方が一般化の余地があって良いかもしれないが，こう定義した方がわかりやすい。}
    $n \in \mathds{Z^{+}}$ に対し，環$\boldsymbol{ \mathds{Z} /n \mathds{Z}}$を，集合として$\mathds{Z} /n \mathds{Z} = \{\bar{0},\bar{1}, \dots ,\overline{n-1} \}$と定義する．$0\leqq x,y \leqq n-1$である整数$x,y$により$\bar{x},\bar{y}$という形をした$\mathds{Z}/n\mathds{Z}$の2元に対し，$x+y$を$n$で割った剰余が$r$ならば，$\bar{x}+\bar{y}=\bar{r}$と定義する．$\bar{x}\bar{y}$も$xy$により同様に定義する．
\end{dfn}

ここで，$\mathds{F}_p=\mathds{Z}/p\mathds{Z} \, (pは素数)$を考えると，$\mathds{F}_p$において，乗法に関する逆元が定義でき，したがって，$\mathds{F}_p$は体となる(証明は，$\mathds{F}_p$係数一次方程式の$\mathds{F}_p$における解が一意に存在することを示せば良い)。そして，明らかに$\mathrm{ch}(\mathds{F}_p)=p$となる。

再びフェルマーの小定理を思い出そう。次のことがわかる：

\begin{mybox}[フロベニウス写像の不動点]
    $\mathds{F}_p$は標数$p$の体であるから，フロベニウス自己準同型$\mathrm{Frob}_p:\mathds{F}_p\ni x\mapsto x^p\in \mathds{F}_p$がある．フェルマーの小定理より，$\mathds{F}_p$において，任意の$x\in \mathds{F}_p$に対して$x^p=x$となる．よって，$\mathrm{Frob}_p=\mathrm{id}_{\mathds{F}_p}$ である．
\end{mybox}
