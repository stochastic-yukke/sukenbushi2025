...
\begin{thm}[\textbf{二平方和定理}]
    奇素数$p$が$x,y\in \mathds{Z}$により$p=x^2+y^2$と表せるのは，$p=2$または$p\equiv 1 \pmod{4}$の時，かつその時に限る．
\end{thm}
...
\begin{dfn}
    整数$n$に対して，$\mathds{Z}[\sqrt{n}]=\{a+b\sqrt{n} \mid a,b \in \mathds{Z}\}$とする．
\end{dfn}

$p=x^2+ny^2$が成り立つことは，$\mathds{Z}[\sqrt{-n}]$において式\eqref{break_down}のような$p$の分解が存在することと対応していることがわかる。分解を考えるのだから，この$\mathds{Z}[\sqrt{n}]$でも，素因数分解的なことができたら嬉しい。できるだろうか。例を見てみよう：

\begin{mybox}[{$\mathds{Z}[\sqrt{-1}]$}]
    $\mathds{Z}$においては，5は素数で，これ以上素因数分解できないが，$\mathds{Z}[\sqrt{-1}]$では，$5=\left(2+\sqrt{-1}\right)\left(2-\sqrt{-1}\right)$となり，分解できる．5は素数にあたるものではない．

    一方で，3や7は，$\mathds{Z}[\sqrt{-1}]$においてもこれ以上``素因数分解''できないため，素数にあたるものであるといえる．
\end{mybox}

\begin{mybox}[{$\mathds{Z}[\sqrt{-13}]$}]
    $\mathds{Z}$では，14は$2\times 7$と一意に分解するが，$\mathds{Z}[\sqrt{-13}]$においては，$14=2\times 7=(1+\sqrt{-13})(1-\sqrt{-13})$と二通りに分解してしまう。``素因数分解''が一意ではないのだ！！
\end{mybox}
...
素因数分解の一意性は，「素数$p$が整数$a, b$の積$ab$を割り切るなら，$p$は$a$と$b$の少なくとも1方を割り切る」という性質から示せるのだが，この性質は一般に$\mathds{Z}[\sqrt{n}]$では成り立たない。したがって，次の定義が必要になる：

\begin{dfn}
    $\alpha\in\mathds{Z}[\sqrt{n}]$とする．「$\alpha=\beta\gamma\,\left(\beta, \gamma\in\mathds{Z}[\sqrt{n}]\right)$と表せるならば，$\beta$は1を割り切る，または$\gamma$は1を割り切る」をみたすとき，$\alpha$を\textbf{既約元}という．「$\alpha$が$\beta\gamma\left(\beta, \gamma\in\mathds{Z}[\sqrt{n}]\right)$を割り切るならば，$\alpha$は$\beta$と$\gamma$の少なくとも一方を割り切る」をみたすとき，$\alpha$は\textbf{素元}という．
\end{dfn}

1を割り切るものというのは，$\mathds{Z}[\sqrt{-1}]$においては$\pm 1, \pm\sqrt{-1}$であり，$\mathds{Z}[\sqrt{-3}]$においては$\pm 1, \pm\frac{1\pm\sqrt{-3}}{2}$であり，それ以外の$n$では$\pm 1$のみである。

また，素元は既約元である(この証明はそこまで難しくない)。一方で，必ずしも既約元は素元にはならない。この性質は後で使う。

\vspace{10pt}

$p=x^2+ny^2$が成り立つことは，$\mathds{Z}[\sqrt{-n}]$において式\eqref{break_down}のような$p$の分解が存在することと対応していた。では，いつ\eqref{break_down}のような分解が存在するのだろうか。それを知るために初等整数論の事実(知っている人も多いだろう)を使う。

\begin{dfn}[平方剰余]
    $p$を素数とし，$a\in\mathds{F}_p$は0でない元とする．$x$についての方程式$x^2=a$が$\mathds{F}_p$で整数解をもつとき，$a$は$p$の\textbf{平方剰余}であるという．
\end{dfn}
...
\begin{dfn}[位数，原始根]\label{def_genshikon}
    $p$を素数とする．$\mathds{F}_p$において，$x^m=1$となる最小の正整数$m$を法$p$における$x$の\textbf{位数}と呼ぶ．また，$\mathds{F}_p$において，整数$u$の位数が$p-1$である，すなわち$p-1$乗してはじめて$u$が1になるとき，$u$を$p$の\textbf{原始根}という．
\end{dfn}

証明はしないが，任意の素数$p$に対して原始根は存在する。また，次の2つの補題も重要だが，証明は省く。

\begin{prop}[原始根の性質]
    $u$を$\mathds{F}_p$における原始根とすると，$p-1$個の元$1,u,u^2,\ldots,u^{p-2}$は$\mathds{F}_p$の0でないすべての元を1つずつ与える．つまり，原始根は，群$\mathds{F}_p^{\times}=\{1,u,u^2,\ldots,u^{p-2}\}$の生成元である．
\end{prop}
...
以下の補題\ref{hojyu_joyo}の証明では$\mathds{F}_p$において考える。
...
$p=x^2+2y^2$と表せる条件や$p=x^2-2y^2$と表せる条件，$p=x^2+xy+y^2$と表せる条件，など色々考えられるが，いま二平方和定理について考えたのと同じように扱うことができてしまう。すなわち，オイラーの規準をはじめとする平方剰余に関する命題を用いて，$\mathds{Z}$を広げた世界での\textbf{素数の分解のしかた}を考えるのである。
...
\begin{mybox}[クンマーのIdea]
    Kummerのアイデアは一言で言えば，``より細かく分解しよう''ということである。例えば，上の方で取り上げた$\mathds{Z}[\sqrt{-13}]$における209の分解を再考しよう：209の規約因子11,19について，さらに\[
    ...
    \]というふうにもできる，と捉え直すのである。このようにすれば，互いに真に異なる既約元分解であっても，有限個の`より細かい因子' $\mathfrak{p}_i$により一意に分解されている，と考える思想だ。Kummerは，上記のような`より細かい因子'の積で表される数を\textbf{理想数}(\ruby{ideale}{イデアル} \ruby{Zahl}{ツァール})と呼んだ。
\end{mybox}

ちなみに，謎の$p$みたいな文字$\mathfrak{p}$は，フラクトゥーア(ドイツ文字)という文字で，英語の\ruby{p}{ピー}にあたるものだが，「ペー」とドイツ語読みすることが多い。神聖ローマ帝国の宮廷用書体・出版事業に端を発し，第二次世界大戦頃までドイツで使われていたとされる。他にも，\ruby{$\mathfrak{a}$}{アー}，\ruby{$\mathfrak{b}$}{ベー}，\ruby{$\mathfrak{q}$}{クー}など，aからzまで，対応するものが揃っている。
...
\end{prop}
ここで，$(n)=\left\{nx\mid x\in\mathds{Z}\right\}$，$(n, m)=\left\{nx+my\mid x, y\in\mathds{Z}\right\}$とする。すると，上記命題は，$(a, b)=(\mathrm{GCD}(a, b))$と表現できる。このように数の集合として捉えることは，整数論における勘所である。というわけで，$(n)$や$(n, m)$といった集合の性質をうまいこと抽象したものとして，Dedekindによるイデアルという概念が登場する：
...
\begin{mybox}[既約元分解の復元]
    $\mathds{Z}[\sqrt{-13}]$において，209の素イデアルによる分解を考える。$\mathfrak{p}_1=(11,14+\sqrt{-13}),\; \mathfrak{p}_2=(11,14-\sqrt{-13}),\; \mathfrak{p}_3=(19,14+\sqrt{-13}),\; \mathfrak{p}_4=(19,14-\sqrt{-13})$とおく。すると，$(\alpha,\beta)=(\alpha,\beta,\alpha\gamma),\; (\alpha,\beta)=(\alpha\pm\beta,\beta)$\,(定義から直ちにわかる)を用いて只管に式変形することで，
    \begin{align*}
...
\begin{thm}[素イデアル分解，存在と一意性]\label{prime_ideal_break}
    $\mathds{Z}$を拡大した可換環\footnote{いま我々が考えている$\mathds{Z}$に平方根を加えたものなどを想定しているが，正確には，「代数的整数環」あるいはより一般に「デデキント整域」と書くべきである。}$R$の非自明な任意のイデアル$\mathfrak{a}$は，素イデアルの積\[
...
\subsubsection{\texorpdfstring{$\mathds{Z}[\sqrt{n}]$}{Z[sqrt{n}]}をちゃんと定義する}\label{zn_teigi}

今まで$\mathds{Z}[\sqrt{n}]$という集合を当たり前のように扱ってきたが，今一度しっかり定義しよう：

ここで，二平方和定理の証明の後に書いた「注目すべき点」の2.を見てほしい。これから考える素数の分解の一般論においても，特定の根を持つ多項式を考えるとうまくいく可能性が高いのだろう，というモチベーションを持って考えていく。

まず，$\mathds{Z}$より一回り`大きい'$\mathds{Q}$に数を加えることから考える。$\mathds{Q}$係数の既約多項式の根となる数を$\alpha$とおき，$\mathds{Q}(\alpha)$を，$\mathds{Q}$を包含し$\alpha$を元として持つ最小の体とする。例えば，$\mathds{Q}(\sqrt{-1})=\left\{x+y\sqrt{-1}\mid x,y\in\mathds{Q}\right\}$，$\mathds{Q}(\sqrt[3]{2})=\left\{x+y\sqrt[3]{2}+z\sqrt[3]{4}\mid x,y,z\in\mathds{Q}\right\}$である。同様に，一般の体$K$に対して考えると，必ず$K(\alpha)$という体が存在することが知られている。

ここで少し環と体についての用語などを整理しておこう：
\begin{itemize}
\item 体$K$に対して，上のようにして構成した体$K(\alpha)$を，$K$に$\alpha$を添加した体と呼ぶ。
\item $\mathds{Q}$と$\mathds{Q}(\alpha)$のように，体$L$の部分環$K$が体であるとき，$K$は$L$の\textbf{部分体}，$L$は$K$の\textbf{拡大体}，$L/K$を\textbf{体の拡大}という。また，体の拡大$L/K$に対し，$L$を$K$上のベクトル空間とみた時の次元を$[L:K]$と書き，$L$の$K$上の\textbf{拡大次数}という。例えば，$\mathds{Q}(\sqrt{-1})=\left\{x+y\sqrt{-1}\mid x,y\in\mathds{Q}\right\}$より，$[\mathds{Q}(\sqrt{-1})/{\mathds{Q}}]=2$，$\mathds{Q}(\sqrt[3]{2})=\left\{x+y\sqrt[3]{2}+z\sqrt[3]{4}\mid x,y,z\in\mathds{Q}\right\}$より，$[\mathds{Q}(\sqrt[3]{2})/{\mathds{Q}}]=3$である。$[L:K]=d\in\mathds{Z}^{+}$であるとき，$L/K$は\textbf{$d$次拡大}であるという。
\item $\mathds{Q}$の拡大体は必ず$\mathds{Q}(\alpha)$の形であり，これを\textbf{代数体}という。
\item 環$R$に対して$R$係数多項式で変数を$x$とするもの全体を$R[x]$とかく。
\item 体の有限次拡大$L/M,\; M/K$に対して，$[L:K]=[L:M][M:K]$が成立する。
\end{itemize}

それでは，代数体を土台にして，うまいこと多項式を扱えるような$\mathds{Z}$を拡大した集合を考えよう：

\begin{dfn}[最小多項式]
    $\alpha\in\mathds{C}$を根に持つ次数が最小の$\mathds{Q}$係数モニック$\in\mathds{Q}[x]$を，\textbf{$\mathds{Q}$上の最小多項式}と呼ぶ．一般に，体$K$に対して，$\alpha\in\mathds{C}$を根に持つ次数が最小の$K$係数モニックを，\textbf{$K$上の最小多項式}と呼ぶ．$\alpha\in\mathds{C}$の最小多項式を$\phi_{\alpha}(x)$と書くことにする．
\end{dfn}

\begin{dfn}
    $K=\mathds{Q}(\alpha)\, (\alpha\text{は}\mathds{Q}\text{係数の既約方程式の解})$とする(つまり$K$は代数体)．このとき，\[
    \mathcal{O}_K\coloneqq \left\{\beta\in K\mid \phi_{\beta}(x)\text{の係数はすべて整数}\right\}
    \]とし，$\mathcal{O}_K$を$K$の\textbf{整数環}と呼ぶ．
\end{dfn}

この定義により，今まで濫用されてきた$\mathds{Z}[\sqrt{n}]$を真っ当に扱えるようになった。詳しく調べてみよう：

\begin{mybox}[$\mathds{Q}(\sqrt{n})$の整数環]
    \begin{enumerate}
        \item $\mathcal{O}_{\mathds{Q}(\sqrt{-1})}=\left\{\alpha\in\mathds{Q}(\sqrt{-1})\mid \phi_{\alpha}(x)\text{の係数は全て整数}\right\}=\left\{a+b\sqrt{-1}\mid a,b\in\mathds{Z}\right\}\eqqcolon \mathds{Z}[\sqrt{-1}]$となる($\alpha=a+b\sqrt{-1}\in\mathds{Q}(\sqrt{-1})$に対し，$\phi_{\alpha}(x)=x^2-2ax+(a^2+b^2)$がわかるから，$\phi_{\alpha}(x)$の係数がすべて整数$\Leftrightarrow 2a\in\mathds{Z},\, a^2+b^2\in\mathds{Z}\Leftrightarrow a,b\in\mathds{Z}$となり，二個目の等号が成立)。この例から分かるように，$\mathds{Z}[\alpha]$とは，$\mathds{Z}$を包含し，$\alpha$を元として持つ最小の環のこと\footnote{明示的に表すのであれば，$\mathds{Z}[\alpha]=\left\{f(\alpha)\mid f(x)\in\mathds{Z}[x]\right\}$と定義するのがよい。同様に，$\mathds{Q}(\alpha)=\left\{f(\alpha)\mid f(x)\in\mathds{Q}[x]\right\}$である。$\mathds{Q}$に対して小括弧($\cdot$)を使い，$\mathds{Z}$に対して大括弧[$\cdot$]を使うところに，体と環との差異に対応する有理式的構造(有理関数体)と多項式的構造(多項式環)との差異が現れている。}である。
        \item 同様に，$\mathcal{O}_{\mathds{Q}(\sqrt{2})}=\left\{a+b\sqrt{2}\mid a,b\in\mathds{Z}\right\}=\mathds{Z}[\sqrt{2}]$, \\$\mathcal{O}_{\mathds{Q}(\sqrt[3]{2})}=\left\{a+b\sqrt[3]{2}+c\sqrt[3]{4}\mid a,b,c\in\mathds{Z}\right\}=\mathds{Z}[\sqrt[3]{2}]$となる。
        \item ところが\dots\dots$\mathcal{O}_{\mathds{Q}(\sqrt{-3})}=\left\{\alpha=a+b\sqrt{-3}\in\mathds{Q}(\sqrt{-3})\mid \phi_{\alpha}(x)\text{の係数はすべて整数}\right\}=\left\{a+b\sqrt{-3}\mid 2a\in\mathds{Z},\, a^2+3b^2\in\mathds{Z}\right\}$となるが，$\frac{\pm 1+\sqrt{-3}}{2}$という数が$2a\in\mathds{Z},\, a^2+3b^2\in\mathds{Z}$を満たすから，$\mathcal{O}_{\mathds{Q}(\sqrt{-3})}=\left\{a+b\frac{1+\sqrt{-3}}{2}\mid a,b\in\mathds{Z}\right\}=\mathds{Z}\left[\frac{1+\sqrt{-3}}{2}\right]$となる!
        \item より一般に，平方因子を持たない$n$に対し$K=\mathds{Q}(\sqrt{n})$を考えると，
        \begin{equation*}
            \mathcal{O}_K=
            \begin{cases*}
                \mathds{Z}[\sqrt{n}]&($n\not\equiv 1\pmod{4}$のとき),\\
                \mathds{Z}[\frac{1+\sqrt{n}}{2}]&(otherwise).
            \end{cases*}
        \end{equation*}
    \end{enumerate}
\end{mybox}
...
\begin{thm}[イデアルの対応定理]
    $I$を可換環$R$の非自明なイデアル，$f\colon R\rightarrow R/I$を自然な射影とする．$R/I$のイデアル全体の集合を$\mathds{X}$，$I$を含む$R$のイデアル全体の集合を$\mathds{Y}$とするとき，\[
    \pi\colon\mathds{X}\ni J'\mapsto f^{-1}(J')\in\mathds{Y},\;\; \psi\colon\mathds{Y}\ni J\mapsto f(J)\in\mathds{X}
    \]は互いに逆写像であり，したがって，$\mathds{X,Y}$は集合として1対1に対応する．
\end{thm}
...
ここで，$f(x)\mapsto f(\alpha)$という代入操作を写像として考える。\ref{zn_teigi}の「$\mathds{Q}(\sqrt{n})$の整数環」の脚注に書いたことを参考にしてもらえば，この代入操作が環の全射準同型$\mathds{Q}[x]\rightarrow \mathds{Q}(\alpha)$を与えることがわかると思う。この写像の核を考える：
\begin{quote}
    $f(\alpha)=0$をみたす$f(x)\in\mathds{Q}[x]$を$\phi_{\alpha}(x)$で割った剰余を$r(x)$とする。このとき，$\phi_{\alpha}$の次数は$r$の次数より大きく，$r(\alpha)=0$であるから，$\phi_{\alpha}(x)$が最小多項式であることとあわせて$r(x)=0$を得る。したがって，$f(x)\in(\phi_{\alpha}(x))$となり，代入操作の写像の核は$\phi_{\alpha}(x)$で生成される単項イデアルである。
\end{quote}
よって，第一同型定理より，同型
\begin{equation}\label{dokei_sp}
    \mathds{Q}[x]/(\phi_{\alpha}(x))\cong \mathds{Q}(\alpha)
\end{equation}
を得る。このような最小多項式を絡めた同型は後々大切になってくる。

\subsubsection{イデアルについての中国式剰余定理}
初等整数論または$\mathds{Z}/{m\mathds{Z}}$における中国式剰余定理(CRT)を知っている人は多いと思うが，より一般のイデアルに対してのCRTを紹介する。
...
$K=\mathds{Q}(\sqrt{n})$\, ($n\in\mathds{Z}\setminus\left\{0,1\right\}$は平方因子を含まない)の整数環での素数の分解を考えよう。
...
\begin{cases*}
    ...
    \sqrt{n}&($n\not\equiv 1\pmod{4}$のとき)
\end{cases*}
\]とすると，$\mathcal{O}_K=\mathds{Z}[\alpha_n]$となる(\textit{c.f}: \ref{zn_teigi}の「$\mathds{Q}(\sqrt{n})$の整数環」のところ)。また，体上の最小多項式と同様にして定義される\textbf{$\mathds{Z}$上の$\alpha_n$の最小多項式}$\in\mathds{Z}[x]$について，\[
\phi_{\alpha_n}(x)=
\begin{cases*}
...
\mathds{Z}[x]/{(\phi_{\alpha_n}(x))}\cong\mathcal{O}_K
\end{equation}
が得られる。

それでは，素数$p$の分解を考えていこう。$\bar{\phi}_{\alpha_n}(x)\in\mathds{F}_p[x]$を，$\phi_{\alpha_n}(x)$の係数を$\mathds{F}_p$の元とみなしたものとすると，同型\eqref{dokei_zx}と第三同型定理より，
\begin{equation}\label{joyokan_ok}
    \mathcal{O}_K/{p\mathcal{O}_K}\cong\mathds{Z}[x]/{(p,\phi_{\alpha_n}(x))}\cong \mathds{F}_p[x]/{(\bar{\phi}_{\alpha_n}(x))}
\end{equation}
となる。ここで，$\bar{\phi}_{\alpha_n}(x)$の$\mathds{F}_p[x]$での分解は次の通り：
\begin{equation*}
    \bar{\phi}_{\alpha_n}(x)=
    \begin{cases*}
        (x-a_1)(x-a_2)&(ただし$a_1\ne a_2$),\\
...
1つ目のパターン(完全分解)では，第一同型定理よる同型$\mathds{F}_p[x]/{(x-a_i)}\ni f(x)\xmapsto{\cong} f(a_i)\in\mathds{F}_p$と，CRTから，$\mathds{F}_p[x]/{(\bar{\phi}_{\alpha_n}(x))}\cong\mathds{F}_p[x]/{((x-a_1)(x-a_2))}\cong\mathds{F}_p[x]/{\overbrace{(x-a_1)(x-a_2)}^{\text{イデアルの積}}}\overset{\text{CRT}}{\cong}\mathds{F}_p[x]/{(x-a_1)}\times\mathds{F}_p[x]/{(x-a_2)}\cong \mathds{F}_p\times\mathds{F}_p$となる(同型\eqref{crt_apply}と$p\mathcal{O}_K$の分解から求めても同じ結果になる)。3つ目のパターン(惰性)で$\mathcal{O}_K/{p\mathcal{O}_K}$は，$\mathds{F}_p$に$\bar{\phi}_{\alpha_n}(x)$の根を添加して得られる$\mathds{F}_p$の唯一の2次拡大$\mathds{F}_{p^2}$と同型になる。
したがって，各パターンで$\mathcal{O}_K/{p\mathcal{O}_K}\cong \mathds{F}_p[x]/{(\bar{\phi}_{\alpha_n}(x))}$は，以下と同型である：
\begin{equation*}
    \mathcal{O}_K/{p\mathcal{O}_K}\cong
    \begin{cases*}
        \mathds{F}_p\times\mathds{F}_p,&(分解する)\\
        \mathds{F}_p[x]/(x-b)^2,&(分解しない)\\
        \mathds{F}_{p^2}&(分解せず$\mathds{F}_p$が拡大する).
    \end{cases*}
\end{equation*}

二平方和定理の証明の後の「注目すべき点」1.に書いたことを思い出していただきたい。抽象代数の視点から素数の分解を捉え直したが，同じ現象が見られたことがわかるだろう。

上記の内容を踏まえ，二平方和定理の証明の舞台でもあった$\mathcal{O}_K=\mathds{Z}[\sqrt{-1}]$の場合を考えてみよう。

まず，$\phi_{\alpha_{-1}}(x)=x^2+1$だから，素数$p$の分解を考えるためには，$\mathds{F}_p[x]$の元としての$x^2+1$(つまり$\bar{\phi}_{\alpha_{-1}}$)の分解を調べればよい。例えば$\mathds{F}_5[x]$において$x^2+1=(x-2)(x+2)$となるが，$\bar{\phi}_{\alpha_{-1}}$が分解するかどうかは，$x^2+1$が$\mathds{F}_p$に根を持つかどうかに依存する。これは，$-1$が$p$の平方剰余かどうか，という話である。したがって，平方剰余の第一補充法則より，$p\equiv 1\pmod{4}$の時かつその時に限り，$\bar{\phi}_{\alpha_{-1}}$は1次式の積に分解し，$p$は$\mathcal{O}_K$で完全分解する。

また，$p$が完全分岐するのは$\mathds{F}_p[x]$で$x^2+1=(x+a)^2$となる時だが，このとき$p=2$となる(係数を比較するだけでわかる)。実際，$\mathds{F}_2[x]$において$x^2+1=(x+1)^2=(x-1)^2$である(\textsf{FD}だね)し，$2\mathcal{O}_K=(\sqrt{-1}+1)^2=(\sqrt{-1}-1)^2$である。

次は，$\omega\coloneqq\alpha_{-3}-1$として，$\mathcal{O}_K=\mathds{Z}[\omega]$(\textbf{アイゼンシュタイン整数環})を簡単に見てみよう。

まず，$\mathds{Z}[\sqrt{-1}]$のときと同様にして，$p$が$\mathcal{O}_K$で分解するかどうかは，$-3$が$p$の平方剰余かどうかによって調べれば良い。ここで，平方剰余の第一補充法則と，二平方和定理の証明の後の「注目すべき点」1.に載せた平方剰余の相互法則を用いる：
\begin{align*}
...
\begin{quote}
    $p$を奇素数とする。$\mathcal{O}_K=\mathds{Z}[\alpha_n]$において，$p$が$n$を割り切る時$p$は完全分岐する。また，$n$が$p$の平方剰余となることと$p$が完全分解することは同値である。したがって，$p$が$n$を割り切らない時，$n$が$p$の平方剰余でないことは$p$が惰性することと同値である。
\end{quote}
