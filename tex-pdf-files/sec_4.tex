...
L=K(\alpha)=\left\{\frac{g(\alpha)}{f(\alpha)}\setmid f(x),g(x)\in K[x],\; f(\alpha)\ne 0\right\}
\]となる$\alpha\in L$が存在することが知られている。
\begin{dfn}[ガロア拡大，ガロア群]
    代数体の有限次拡大$L/K$に対し，$L=K(\alpha)$を満たす$\alpha\in L$をとる。$\alpha$の$K$上の最小多項式$\phi_{\alpha}(x)$のすべての根$\alpha=\alpha^{(1)},\alpha^{(2)},\ldots,\alpha^{(n)}$が$L$の元となる時，$L/K$は\textbf{ガロア拡大}であるという．ガロア拡大$L/K$において，$K$への制限が恒等写像で，$L$上の体自己同型であるもの全体の集合を$\mathrm{Gal}(L/K)$と書き，$L/K$の\textbf{ガロア群}と呼ぶ．
\end{dfn}
...
\begin{itemize}
    ...
    \item $\mathds{Q}(\sqrt[3]{2})/{\mathds{Q}}$はガロア拡大ではない代数体の有限次拡大である(証明しよう)。
\end{itemize}
...
\begin{dfn}[円分体]
    $\zeta_n\coloneqq e^{\frac{2\pi\sqrt{-1}}{n}}$を1の原始$n$乗根とする．$\mathds{Q}$に$\zeta_n$を添加した$\mathds{Q}(\zeta_n)$を\textbf{($n$次)円分体}という．
\end{dfn}
\begin{dfn}[オイラー関数]
    $(\mathds{Z}/{n\mathds{Z}})^{\times}$を$\mathds{Z}/{n\mathds{Z}}$の乗法群($n$と互いに素な整数$a$の剰余類を集めたもの)とする．$\varphi\colon\mathds{Z}^+\ni n\mapsto\# (\mathds{Z}/{n\mathds{Z}})^{\times}\in\mathds{Z}^+$を\textbf{オイラー関数}と呼び，必ず$\varphi$で表すことにする．
\end{dfn}
\begin{dfn}[円分多項式]
    $\varphi(n)$次多項式\[
    \Phi_n(x)=\prod_{k+n\mathds{Z}\in(\mathds{Z}/{n\mathds{Z}})^{\times}}(x-\zeta_n^k)
    \]は$\mathds{Z}$上既約な$\mathds{Z}[x]$の元であるが，これを\textbf{円分多項式}と呼ぶ．
\end{dfn}
円分多項式は，$\mathds{Z}$上の最小多項式となることが知られている。したがって，$\Phi_n(x)=\phi_{\zeta_n}(x)$である。また，$\mathcal{O}_{\mathds{Q}(\zeta)}=\mathds{Z}[\zeta]$も成り立つ。

なぜ円分体なるものを定義するのだろうか?モチベーションを得るために紹介する事柄がある：
\begin{mybox}[ガウス和]
    $\mathds{Q}(\sqrt{-1})=\mathds{Q}(\zeta_4)$より，$\mathds{Q}(\sqrt{-1})$は4次円分体である。実は，例えば$\mathds{Q}(\sqrt{-7})$のように円分体でないものも，円分体との関係がある：$\sqrt{-7}=(\zeta_7+\zeta_7^2+\zeta_7^4)-(\zeta_7^3+\zeta_7^5+\zeta_7^6)$という等式があり，$\mathds{Q}(\sqrt{-7})\subset\mathds{Q}(\zeta_7)$となるのだ。このように，奇素数$p$に対して\[
    G_p\coloneqq\sum_{j=1}^{p-1}\left(\frac{k}{p}\right)e^{\frac{2\pi\sqrt{-1}}{p}j}
...
\end{mybox}
他にもモチベーションになり得る事柄は数多あるが，先を急ごう。$\mathds{Q}(\zeta_n)/{\mathds{Q}}$を\textbf{円分拡大}という。円分拡大について少し見ておこう：

まず，定義からわかるように，$\zeta_n$の最小多項式である円分多項式$\Phi_n(x)$は，$\zeta_n^k\,(k\text{と}n\text{は互いに素})$を根に持つ。$\mathds{Q}(\zeta_n)/{\mathds{Q}}$はガロア拡大となり，$\#\mathrm{Gal}(\mathds{Q}(\zeta_n)/{\mathds{Q}})=\varphi(n)$となる。ここで，$\sigma_k\colon\zeta_n\mapsto\zeta_n^k\, (k\in (\mathds{Z}/{n\mathds{Z}})^{\times})$を考えると，$(\mathds{Z}/{n\mathds{Z}})^{\times}\ni k+n\mathds{Z}\mapsto\sigma_k\in\mathrm{Gal}(\mathds{Q}(\zeta_n)/{\mathds{Q}})$は群同型である。ここで，ガロアの基本定理から$\mathds{Q}(\zeta_n)$の部分体は，$\mathrm{Gal}(\mathds{Q}(\zeta_n)/{\mathds{Q}})$の部分群$H$の不変体$\mathds{Q}(\zeta_n)_H=\{x\in\mathds{Q}(\zeta_n)\mid\forall g\in H,\, g(x)=x\}$として与えられる。難しく思われる「中間体を求める」というタスクを，「群の構造を調べる」比較的容易な作業に落とし込めるのが，ガロア理論の強さである。
...
\begin{itemize}
    \item 位数の等しい有限体はすべて同型で，任意の有限体はある素数$p$に対する$\mathds{F}_{p^n}$($n$はその有限体の位数に等しい)と同型である。
    \item $q,q'$を素数のべき乗とする。$\mathds{F}_{q'}$が$\mathds{F}_{q}$の拡大体となるのは，$q'$が$q$で割り切れる時，かつその時に限り，体の拡大$\mathds{F}_{q'}/{\mathds{F}_q}$はガロア拡大である。
\end{itemize}
ここで，久しぶりにフロベニウスが登場する\dots\dots というか，フロベニウスが待ってくれていたのだ。ただいま：

$q,q'$を素数$p$のべき乗とし，$q\mid q'$とすると，ガロア拡大$\mathds{F}_{q'}/{\mathds{F}_q}$のガロア群$\mathrm{Gal}(\mathds{F}_{q'}/{\mathds{F}_q})$は，\textbf{フロベニウス自己同型}$\mathrm{Frob}_q\colon\mathds{F}_{q'}\ni x\mapsto x^q\in\mathds{F}_{q'}$が生成する巡回群$\langle\mathrm{Frob}_q\rangle$となることが知られている。ここで有限体のガロア群の生成元として表れたフロベニウスは，より一般の拡大について考える際にも登場する。
...
フロベニウス元を導入したところで，円分体における素数の分解を考えよう。1の原始$n$乗根を1つ取って$\zeta$とし，円分拡大$\mathds{Q}(\zeta)/\mathds{Q}$を考えると，これはアーベル拡大になることが知られている。また，$n=2m\left(2\nmid m\right)$と表せる時，$\mathds{Q}(\zeta_n)=\mathds{Q}(\zeta_m)$だから，$n$は奇数または4の倍数であるとして良い。この時，次の事実がある(証明省略)：
\begin{prop}
    素数$p$が$\mathds{Q}(\zeta)/\mathds{Q}$で分岐することは，$p\mid n$と同値である．
\end{prop}

不分岐の場合を考えていたから，素数$p$と$n$は互いに素の場合を考える。
\begin{prop}
    $n$と互いに素な素数$p$に関するフロベニウス元$\left(\frac{L/K}{p\mathds{Z}}\right)$は，$\sigma_p(\zeta)=\zeta^p$という写像$\sigma_p\in\mathrm{Gal}(\mathds{Q}(\zeta)/\mathds{Q})$によって定まる．
\end{prop}
\begin{prf}
    $\mathcal{O}_{\mathds{Q}(\zeta)}=\mathds{Z}[\zeta]$とフェルマーの小定理，そして$\mathsf{FD}$より，任意の$x=\sum a_i\zeta^i\in\mathds{Z}[\zeta]$に対し，$\sigma_p(x)+p\mathds{Z}[\zeta]=\sum a_i(\zeta^p)^i+p\mathds{Z}[\zeta]=\sum a_i^p\zeta^{pi}+p\mathds{Z}[\zeta]=(\sum a_i\zeta^i)^p+p\mathds{Z}[\zeta]=x^p+p\mathds{Z}[\zeta]$となる．これを満たす写像は一意に定まり，$\sigma_p$はフロベニウス元を定める．
\end{prf}
$\sigma_p$の$\mathrm{Gal}(\mathds{Q}(\zeta)/\mathds{Q})$における位数が$f=\# Z_{p\mathds{Z}}=\#\left\langle\left(\frac{L/K}{p\mathds{Z}}\right)\right\rangle$を与えるから，$\sigma_p$の位数を求めたい。ここで，同型による対応$\mathrm{Gal}(\mathds{Q}(\zeta)/\mathds{Q})\ni\sigma_p\mapsto\bar{p}\in (\mathds{Z}/n\mathds{Z})^{\times}$があったから，$\sigma_p$の位数$f$は，$\bar{p}$の$(\mathds{Z}/n\mathds{Z})^{\times}$における位数に等しいことがわかり，次のようになる：
\begin{thm}
    $n$と互いに素な素数$p$に対し，$p^f\equiv 1\pmod{n}$を満たす最小の正整数$f$により，$p$は\[
    p\mathds{Z}[\zeta]=\mathfrak{p}_1\mathfrak{p}_2\cdots\mathfrak{p}_{[\mathds{Q}(\zeta):\mathds{Q}]/f}=\mathfrak{p}_1\mathfrak{p}_2\cdots\mathfrak{p}_{\varphi(n)/f}
    \]と分解する．
\end{thm}
...
\begin{thm}[クロネッカー-ウェーバーの定理]
    $\mathds{Q}$の任意のアーベル拡大は，ある円分体$\mathds{Q}(\zeta)$の部分体となる．
\end{thm}
